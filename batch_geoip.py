...
@contextmanager
def write_to_file(file_path: Path) -> Generator[Callable[[str], int]]:
...
class BatchWorker:
    ...
    def __init__(self: "BatchWorker", host: str) -> None:
        ...
        self.ipquery_url = f"https://api.ipquery.io/{self.host}"
    ...
    def process(self: "BatchWorker") -> None:
        ...
        key_mappings = {
            ...
            "longitude": "lon",
            "asn": "as",
            "isp": "isp",  # Separate 'isp'
            "org": "org",  # Separate 'org'
            "host": "ip",
            "query": "ip",
        }
        ...
        # ipquery.info
        try:
            ipquery_data = self.ipquery()
            add_to_combined_results(ipquery_data)
        except (requests.RequestException, KeyError) as err:
            print(err)
    ...
    def ipquery(self: "BatchWorker") -> dict:
        """Connect to ipquery.info and return the results."""
        return self.connect(self.ipquery_url).json()
